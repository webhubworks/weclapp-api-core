...
class ArticleSupplySourceGetResponse200Normalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('additionalProperties', $data)) {
            $object->setAdditionalProperties($this->denormalizer->denormalize($data['additionalProperties'], \Weclapp\Generated\Model\ArticleSupplySourceGetResponse200AdditionalProperties::class, 'json', $context));
            unset($data['additionalProperties']);
        }
        if (\array_key_exists('result', $data)) {
            $values = [];
            foreach ($data['result'] as $value) {
                $values[] = $this->denormalizer->denormalize($value, \Weclapp\Generated\Model\ArticleSupplySource::class, 'json', $context);
            }
